...
`model-ollama` is an optional reasoning adapter for `agent-core`.
...
```python
from model_ollama import HttpOllamaClient, OllamaReasoner
...
```text
transport-redpanda  -> agent-core <- storage-delta
memory-lancedb   -> agent-core <- model-ollama
```
