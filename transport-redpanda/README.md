...
`transport-redpanda` is a thin adapter that connects Redpanda to the generic `agent-core` runtime.
...
- `transport-redpanda` owns the conversion between Redpanda records and the generic `agent_core.Event` model.
...
[`storage-postgres`](../storage-postgres/README.md).
...
```python
from transport_redpanda import RedpandaProducer
...
```python
from transport_redpanda import RedpandaConsumer
...
`transport-redpanda` is only an event transport adapter. The domain-agent runtime remains in `agent-core`, and the core remains independent from Redpanda and Kafka.
