...
| One stable operation ID creates one business effect. | **Conditional** | `conformance` and Delta evidence reference | Effect owner stores an operation ID durably and atomically enough for its writer topology. | Deduplicate; escalate any contradictory evidence as `CONFLICT`. |
...
| An external provider failure does not create a guessed business result. | **Conditional** | [external effect boundary](external-effect-boundary.md) and `conformance` helpers | The domain persists a stable provider idempotency key and has an authoritative reconciliation route. | Keep `UNKNOWN`; reconcile or escalate. Do not generically retry with a new operation ID. |
...
7. applicable checks from [`conformance`](../conformance/README.md).
...
- [ ] Domain has adopted and extended the
      [`conformance`](../conformance/README.md) checks.
