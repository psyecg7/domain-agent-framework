...
`enterprise` is an optional reference boundary for a separately deployed
...
```bash
python -m enterprise.policy_service --private-key /tmp/policy.pem --public-key-out /tmp/policy.pub --generate-development-key
python -m enterprise.executor_service --public-key /tmp/policy.pub --replay-table /tmp/executor-replay --owned-action-type CREATE_ORDER --effect-log /tmp/executor-effects.jsonl
```
...
```bash
python -m enterprise.executor_service --public-key /tmp/policy.pub \
